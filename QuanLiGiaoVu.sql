...
------------------------Bài thực hành 2
------------------------Bài 4
--Bài 11: 11.	Học viên ít nhất là 18 tuổi.
ALTER TABLE dbo.HOCVIEN ADD CONSTRAINT CHECK_TUOI CHECK( YEAR(GETDATE())-YEAR(NGSINH)>18 )
--Bài 12 : 12.	Giảng dạy một môn học ngày bắt đầu (TUNGAY) phải nhỏ hơn ngày kết thúc (DENNGAY).
ALTER TABLE  dbo.GIANGDAY ADD CONSTRAINT CHECK_SF CHECK(DENNGAY>TUNGAY)
--Bài 13: 13.	Giáo viên khi vào làm ít nhất là 22 tuổi.
ALTER TABLE dbo.GIAOVIEN ADD CONSTRAINT CHECK_TUOIGV CHECK (YEAR(GETDATE())- YEAR(NGSINH)>22)
---Bài 14: 14.	Tất cả các môn học đều có số tín chỉ lý thuyết và tín chỉ thực hành chênh lệch nhau không quá 3.
ALTER TABLE dbo.MONHOC ADD CONSTRAINT CHECK_MHTC CHECK(ABS(TCLT-TCTH) <=3)
-------------------------Bài 6
--1.	In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) lớp trưởng của các lớp.
SELECT H.MAHV, H.HO, H.TEN, H.NGSINH, H.MALOP 
FROM HOCVIEN AS H INNER JOIN LOP AS L ON H.MAHV=L.TRGLOP 
...
FROM HOCVIEN AS H INNER JOIN KETQUATHI AS K ON H.MAHV=K.MAHV 
WHERE K.MAMH='CSDL' AND H.MALOP='K12' 
...
SELECT H.MAHV, H.HO, H.TEN
FROM HOCVIEN AS H INNER JOIN KETQUATHI AS K ON H.MAHV=K.MAHV
WHERE H.MALOP='K11' AND K.MAMH='CTRR' AND K.KQUA='Khong Dat' AND K.LANTHI=1
--5.	* Danh sách học viên (mã học viên, họ tên) của lớp “K” thi môn CTRR không đạt (ở tất cả các lần thi).
SELECT *
FROM HOCVIEN AS H INNER JOIN KETQUATHI AS K ON H.MAHV=K.MAHV
WHERE H.MALOP LIKE 'K%' AND K.MAMH='CTRR' AND KQUA='Khong Dat'